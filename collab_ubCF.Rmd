...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      h3("Recommendations"),
      verbatimTextOutput("recommendations_output"),
      h3("Score"),
...
server <- function(input, output) {
  ...
  score <- eventReactive(input$recommend_btn, {
    req(input$selected_movies)
    common_items_1 <- intersect(recommendations(), y_data)
    common_items_2 <- intersect(recommendations(), common_items_12)
    (length(common_items_1) - length(common_items_2)) / 3
  })
  
  output$recommendations_output <- renderText({
    req(recommendations())
    paste("Top 3 Recommendations: ", paste(recommendations(), collapse = ", "))
  })
  
  output$score_output <- renderText({
    req(score())
    paste("Recommendation Score: ", round(score(), 4))
  })
}
